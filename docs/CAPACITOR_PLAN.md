...
**Reason:** native-like gestures (swipe, dnd) are now implemented directly in the PWA. Native shell is no longer an immediate requirement.
...
**Viability: High**
The current codebase (Next.js, Shadcn UI, Supabase) is highly compatible with Capacitor. The existing focus on "Offline First" (TanStack Query) and Responsive Design (Tailwind CSS) means the bulk of the "mobile work" is already done.
