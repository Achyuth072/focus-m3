...
- _Solid Actions_: High-value primary actions (Create, Save, Start) use solid Kanso Blue backgrounds (`bg-brand`) to signal a deliberate "finishing" stroke. These buttons should feature a subtle shadow (`shadow-brand/10`) and premium transition physics.
...
- **The Horizontal Frame**: Unified padding of **16px (px-4)** on mobile and **24px (px-6)** on desktop to ensure structural consistency and prevent card crowding.
...
- **Hybrid Stack Model (Kanban)**:
  - **Philosophy**: Fusing "Functional Stability" with "Visual Minimalism."
  - **The Column**: The organizational container (column) visually shrinks to fit its children (`max-h-full`) to eliminate empty bordered boxes and preserve "Ma."
  - **The Droppable**: The underlying interaction layer (`setNodeRef`) remains a fixed `h-full` stalk to ensure consistent and stable drag/drop targets even when the list is empty or short.
  - **The Scroll**: Uses `scrollbar-hide` with `-mx-1 px-1` padding compensation to allow task hover-effects (borders/glows) to bleed naturally into the gutter without causing layout jitter.
...
### 7.4 Atomic Layer (Task/Habit Sheet) - The Detail

- **Purpose**: Deep Definition.
- **Mental State**: The "Craftsman." Engaged in the internal components of a single piece of work.
- **Design**: Immersive detail. Drawer/Modal overlay that isolates the task/habit for deep work.
- **Alignment**: Standardized spacing of `px-4 pt-6` for headers, `px-4 py-4 space-y-4` for content, and `p-4` for footers across all creation/edit sheets.
...
### 8.3 Navigation Resilience (Ma in Motion)

To preserve user intent on high-speed mobile devices, we prioritize **Intent Preservation** over immediate feedback:

- **The Intercept Pattern**: Mobile sidebar navigation uses a 50ms defensive delay after the menu begins to close before triggering the router. This prevents the "Transition Cancellation" bug where the unmounting menu swallows the navigation event.
- **Iconographic Semantic Distinction**:
  - **`Layers`**: Reserved for Habits (symbolizing consistency and accumulation).
  - **`CalendarClock`**: Reserved for Do Dates (planned execution).
  - **`Settings`**: Reserved for System configuration.
