...
const YearMonth = memo(({ monthDate, onDateClick }: { monthDate: Date; onDateClick?: (date: Date) => void }) => {
  return (
...
const YearView = memo(({ currentYear, onDateClick, className }: YearViewProps) => {
  const start = useMemo(() => startOfYear(new Date(currentYear, 0, 1)), [currentYear]);
