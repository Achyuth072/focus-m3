...
import { uploadWebDavBackup, downloadWebDavBackup, testWebDavConnection } from "@/lib/backup/webdav-sync";
  ...
  describe("testWebDavConnection", () => {
    it("returns success for valid credentials", async () => {
      ...
      (createDAVClient as any).mockResolvedValueOnce({
        fetchPrincipalUrl: vi.fn().mockResolvedValue("https://dav.example.com/principals/user"),
      });
    ...
    it("returns error for invalid credentials", async () => {
      const { createDAVClient } = await import("tsdav");
      (createDAVClient as any).mockRejectedValueOnce(new Error("401 Unauthorized"));
